...
int render()
{
    ...
    for (float angle = 0; angle <= 360; angle += 1) {
        setVertexRotation(0.4, 0.4, angle + angle_circle, big_circle_center_x, big_circle_center_y, 1.0f, 0.843f, 0.0f); // 회전 각도 적용
    }
    glEnd();
    ...
    glBegin(GL_TRIANGLE_FAN);
    for (float angle = 0; angle <= 360; angle += 1) {
        setVertexRotation(0.1, 0.1, angle + angle_circle, rotated_small_circle_center_x, rotated_small_circle_center_y, 1.0f, 0.91f, 0.76f); // 회전 각도 적용
    }
    glEnd();
    ...
    for (float angle = 0; angle <= 360; angle += 1) {
        setVertexRotation2(0.1, 0.1, angle + angle_circle, rotated_small_circle_center_x, rotated_small_circle_center_y, 1.1f, 0.9273f, 0.0f); // 회전 각도 적용
    }
    glEnd();
